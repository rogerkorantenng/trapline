...
function AppConfig(json) {
    ...
    if (json.sourceIgnores)
        partial.sourceIgnores = json.sourceIgnores;
    if (json.dev) {
        partial.dev = {};
...
export function validate(config) {
    ...
    for (const item of config.menu?.items ?? []) {
        if (item.postFilter === 'currentApp' && item.location.includes('subreddit')) {
            errs.push('`config.menu.items` cannot use `postFilter: "currentApp"` when `location` includes `"subreddit"`');
        }
    }
